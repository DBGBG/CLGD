...
def full_detection(df, instance_name, speed_avg=None):
    """对单个辊道的5分钟窗口进行完整异常检测"""
    detection_results = []
    time_index = df.index
    ...
    # 9. 跳闸报警（修复：不再存current_values，分级告警）
    i = 0
    while i < len(current_values):
    ...
    skip_block = False
    if speed_avg is not None and speed_avg < 20:
        ...
        def process_roller(name, df):
            ...
            if group_name and group_name in speed_data:
                ...
                if speed_avg < 5:
                    ...
                    return name, []
            alerts = full_detection(df, name, speed_avg)
            if alerts:
